  ...
  if (props.speed !== 'undefined') {
    speed =
      ...
      String(props.speed).indexOf("/") === -1 ? (
        <span>{props.speed} sec, </span>
      ) : (
        <span className="frac">{props.speed} sec, </span>
      )
  }
  ...
  return (
    <div id={imageName} className="pane page--image">
      ...
      <p className="image__info u-mb0">
        <b>{props.name.split('.').slice(0, -1).join('.')} </b>
          {props.camera && `${props.camera}, `}
          {props.fStop && `\u0192${props.fStop}, `}
          {speed}
          {props.focalLength && `${props.focalLength}, `}
          <span className="caps">ISO</span>{" "}{props.iso}
      </p>
    </div>
  )
}
